...
public class RefreshableView extends LinearLayout implements OnTouchListener {
	...
	public RefreshableView(Context context, AttributeSet attrs) {
		...
		header = LayoutInflater.from(context).inflate(
				R.layout.ciist_zhn_pull_to_refresh, null, true);
		progressBar = (ProgressBar) header.findViewById(R.id.progress_bar);
		arrow = (ImageView) header.findViewById(R.id.arrow);
		description = (TextView) header.findViewById(R.id.description);
		updateAt = (TextView) header.findViewById(R.id.updated_at);
		// 达到touchslop这个值是来进行刷新
		touchSlop = ViewConfiguration.get(context).getScaledTouchSlop();
		refreshUpdatedAtValue();
		// 设置方位
		setOrientation(VERTICAL);
		// 很多时候需要动态显示一些内容，在动态添加View的时候，主要使用addView方法。
		addView(header, 0);
	}
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if (changed && !loadOnce) {
			...
			// 假设你的ListView列表数据在不需要全部刷新，
			// 只需刷新有数据变化的那一条或者说刷新指定某条数据的情况下，
			// 就可以用getChildAt(index)来获取某个指定position的view，进行刷新。
			// 但是有一点需要注意的是：它只能获取到当前可见的ListView子项。
			list = (ListView) getChildAt(1);
